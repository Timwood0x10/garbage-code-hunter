...
use garbage_code_hunter::{CodeAnalyzer, Reporter, I18n};
use std::fs;
...
fn test_analyzer_with_multiple_exclusions() {
    ...
    let analyzer = CodeAnalyzer::new(&exclusions);
...
fn test_analyzer_with_empty_exclusions() {
    ...
    assert!(!issues.is_empty(), "Should detect issues when no exclusions");
}
...
fn test_analyzer_with_invalid_exclusion_patterns() {
    ...
    let issues = analyzer.analyze_file(&file_path);
    assert!(!issues.is_empty(), "Should still work with invalid exclusion patterns");
}
...
"#;
    ...
    for (harsh, savage, verbose, top, max_issues, summary, markdown, lang) in configurations {
        let reporter = Reporter::new(harsh, savage, verbose, top, max_issues, summary, markdown, lang);
...
fn test_i18n_all_rule_types() {
    ...
    for rule_type in rule_types {
        ...
        assert!(!zh_messages.is_empty(), "Should have messages for {}", rule_type);
        
        // Test English messages
        let en_messages = i18n_en.get_roast_messages(rule_type);
        assert!(!en_messages.is_empty(), "Should have messages for {}", rule_type);
        
        // Test invalid language (should fallback)
        let invalid_messages = i18n_invalid.get_roast_messages(rule_type);
        assert!(!invalid_messages.is_empty(), "Should have fallback messages for {}", rule_type);
    }
}
...
fn test_i18n_all_suggestion_combinations() {
    ...
    let rule_combinations = vec![
        ...
        vec!["terrible-naming".to_string(), "deep-nesting".to_string(), "long-function".to_string()],
        vec!["terrible-naming".to_string(), "single-letter-variable".to_string(), "deep-nesting".to_string(), "long-function".to_string(), "unwrap-abuse".to_string(), "unnecessary-clone".to_string()],
    ];
    
    for rules in rule_combinations {
        let zh_suggestions = i18n_zh.get_suggestions(&rules);
        assert!(!zh_suggestions.is_empty(), "Should have Chinese suggestions for {:?}", rules);
        
        let en_suggestions = i18n_en.get_suggestions(&rules);
        assert!(!en_suggestions.is_empty(), "Should have English suggestions for {:?}", rules);
    }
}
...
fn test_i18n_missing_keys() {
    ...
    for key in missing_keys {
        ...
        assert!(result.contains("Missing translation"), "Should handle missing key: {}", key);
    }
}
...
fn test_analyzer_with_mixed_files() {
    ...
    assert!(issues.iter().all(|issue| issue.file_path.extension().unwrap() == "rs"), 
           "All issues should be from Rust files");
}

#[test]
fn test_severity_and_roast_level_coverage() {
    use garbage_code_hunter::{Severity, RoastLevel};
    ...
    let roast_levels = vec![RoastLevel::Gentle, RoastLevel::Sarcastic, RoastLevel::Savage];
...
fn test_code_issue_debug_and_clone() {
    use garbage_code_hunter::{CodeIssue, Severity, RoastLevel};
