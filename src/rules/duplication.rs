...
use std::path::Path;
use syn::{visit::Visit, File, Block};
...
impl DuplicationVisitor {
    ...
    fn detect_line_duplications(&mut self, issues: &mut Vec<CodeIssue>) {
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            self.line_hashes.entry(normalized).or_insert_with(Vec::new).push(line_num + 1);
        }
        
        // find duplicate lines
        for (_line_content, line_numbers) in &self.line_hashes {
            if line_numbers.len() >= 3 { // 3 times or more duplicate   
                let messages = vec![
    ...
    fn detect_block_duplications(&self, issues: &mut Vec<CodeIssue>) {
        ...
        for (i, block) in self.code_blocks.iter().enumerate() {
            if block.len() > 50 { // only detect larger code blocks
                let signature = generate_block_signature(block);
                block_signatures.entry(signature).or_insert_with(Vec::new).push(i);
            }
        }
