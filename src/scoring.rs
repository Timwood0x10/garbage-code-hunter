...
impl QualityLevel {
    pub fn from_score(score: f64) -> Self {
        match score as u32 {
            81..=100 => QualityLevel::Excellent,
            61..=80 => QualityLevel::Good,
            41..=60 => QualityLevel::Average,
            21..=40 => QualityLevel::Poor,
...
impl CodeScorer {
    ...
    fn calculate_normalized_category_scores(&self, issues: &[CodeIssue], total_lines: usize) -> HashMap<String, f64> {
        let mut category_scores = HashMap::new();
        let mut category_counts: HashMap<String, usize> = HashMap::new();
    ...
    fn calculate_category_score(&self, issue_count: usize, total_lines: usize, category: &str) -> f64 {
        if total_lines == 0 {
        ...
        let (excellent_threshold, good_threshold, average_threshold, poor_threshold) = match category {
        ...
        let score = if issues_per_1k_lines <= excellent_threshold {
            ...
            (issues_per_1k_lines - excellent_threshold) / (good_threshold - excellent_threshold) * 20.0
        } else if issues_per_1k_lines <= average_threshold {
            20.0 + (issues_per_1k_lines - good_threshold) / (average_threshold - good_threshold) * 20.0
        } else if issues_per_1k_lines <= poor_threshold {
            40.0 + (issues_per_1k_lines - average_threshold) / (poor_threshold - average_threshold) * 20.0
        } else {
